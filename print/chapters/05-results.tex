...
\begin{subchapter}{\Acrlong{cfo} estimation}
  ...
  transmitter the \gls{cfo}.

  As \gls{schcox} was the only method presented in this thesis
  thus far providing a means of estimating the \gls{cfo}
  ...
  The figure shows that the carrier frequency were
  estimated to within a range of $\pm\SI{0.7}{\hertz}$.
  The carrier spacings in a multi-carrier system
  would have to designed to tollerate residual
  frequency offsets in that region.
\end{subchapter}
...
\begin{subchapter}{\Acrshort{cpu} load}
  ...
  a preset sequence over and over again while keeping
  track of the number of samples outputted per unit of time.
  ...
  It provides, among others, the avx2 group of instructions
  ...
  of this \gls{cpu} only the sse to sse4.2 line of
  instructions were implemented instead of the
  more powerful avx2 \acrshort{simd} instruction present
  in the desktop \gls{cpu}. \\

  The samplerates archieved in this test are shown
  in \autoref{tab:cpuload} under the ``Benchmark samplerate''
  column.
  ...
  in \autoref{img:benchmarkhtop}: only a small number of
  processor cores are utilized by the benchmarking program
  ...
  two cores utilized by \gls{schcox} components can
  also be seen as a feature, whereby the remaining cores
  may still be used for other processing blocks without impacting
  the overall archievable samplerate.
\end{subchapter}
